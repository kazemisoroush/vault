...
func TestConverseWithContentSendsItAndReturnsTheReply(t *testing.T) {
	// Arrange: a one-shot call with rich content and no tools, the shape extraction uses.
	...
	// Assert: the reply comes back, the call is recorded, and no tools were declared.
	require.NoError(t, err)
...
func TestConverseRecordsAFailedCall(t *testing.T) {
	// Arrange
	...
	// Act
	_, err := model.Converse(context.Background(), Conversation{Prompt: "a prompt", MaxTokens: 100})
